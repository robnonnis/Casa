...
const s = {
  ...
  roomPhoto: { width:"100%", height:200, objectFit:"cover", borderRadius:14, marginBottom:14, display:"block" },
  ...
  return (
    <div style={s.affrescoCard}>
      ...
      <div style={s.affrescoText}>
        <div style={{fontFamily:"'Cormorant Garamond', Georgia, serif", fontSize:20, color:c.sand, fontWeight:400, marginBottom:6}}>🎨 Affreschi originali anni '50</div>
        <p style={{fontSize:13, color:"rgba(245,240,232,0.8)", lineHeight:1.7, margin:0}}>I soffitti sono affrescati a mano da artigiani sardi. Un patrimonio artistico unico che rende Casa Uta davvero speciale.</p>
      </div>
    </div>
  );
}
    ...
    <div style={s.app}>
      ...
      <div style={s.hero}>
        ...
        <div style={s.heroContent}>
          ...
          <div style={{fontSize:10, letterSpacing:"3.5px", textTransform:"uppercase", color:"rgba(245,240,232,0.5)", marginTop:10, fontFamily:"'Jost', sans-serif", fontWeight:200}}>Casa Uta — Via Cimitero 38</div>
  ...
  return <div style={s.app}>
    ...
    <div style={s.hero}>
      ...
      <div style={s.heroContent}>
        ...
        <h2 style={{...s.pageTitle, color:c.cream, margin:0}}>Benvenuti a Casa Uta</h2>
      </div>
    </div>
    <div style={s.content}>
      <AffrescoCard/>
...
function Appartamento({go}) {
  const rooms = [
    {
      icon:"🍳", name:"Cucina e soggiorno",
      photos:[PHOTOS.cucinaSoggiorno, PHOTOS.cucina],
      items:["Piano cottura a induzione","Forno elettrico","Lavastoviglie","Frigorifero","Cappa aspirante","Climatizzatore Wi-Fi","Tavolo allungabile + 4 sedie","Divano letto 3 posti","Vetrata su veranda","Soffitto affrescato (rosa)"]
    },
    {
      icon:"🛏", name:"Camera da letto",
      photos:[PHOTOS.camera],
      items:["Letto matrimoniale con contenitore","Armadio ~4 metri","Settimino","Comodini con applique","Condizionatore Wi-Fi","Soffitto affrescato (geometrico oro)"]
    },
    {
      icon:"🚿", name:"Bagno",
      photos:[PHOTOS.bagno1, PHOTOS.bagno2],
      items:["Box doccia 80×100 cm","Colonna doccia con massaggio","Mobile lavandino con cassettoni","Specchio con illuminazione LED","Ventilazione automatica"]
    },
    {
      icon:"🌿", name:"Veranda e giardino",
      photos:[PHOTOS.veranda, PHOTOS.verandaCartile],
      items:["Veranda coperta ~40 m²","Accesso diretto dalla vetrata scorrevole","Giardinetto privato","Ideale per colazioni all'aperto"]
    },
  ];
  ...
  return <div style={s.app}>
    ...
    <div style={s.content}>
      ...
      {rooms.map(({icon,name,photos,items})=>(
        <Card key={name} style={{padding:"0 0 16px", overflow:"hidden"}}>
          {/* Foto: se sono 2 le affianca, se è 1 va full width */}
          {photos.length === 1
            ? <img src={photos[0]} alt={name} style={{width:"100%", height:200, objectFit:"cover", display:"block", marginBottom:16}} />
            : <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:2, marginBottom:16}}>
                {photos.map((p,i)=><img key={i} src={p} alt={`${name} ${i+1}`} style={{width:"100%", height:150, objectFit:"cover", display:"block"}}/>)}
              </div>
          }
...
function Dintorni({go}) {
  return <div style={s.app}>
    <PageHead title="I dintorni" back={()=>go("home")} icon={<Ic.grid/>}/>
    <div style={s.content}>
      <div style={s.hlBox}><div style={s.hlTitle}>Posizione strategica</div><p style={{fontSize:14,lineHeight:1.7,opacity:0.92,margin:0}}>Nel raggio di 300 metri trovate tutto il necessario: supermercati, farmacia, bar e ristoranti.</p></div>
      <Card><CT text="💊 Salute"/>
      ...
      <Card><CT text="🛒 Spesa"/>
        ...
        <p style={{fontSize:11,color:c.muted,padding:"2px 0 0"}}>Orario continuato 8:30–20:00</p>
      </Card>
      <Card><CT text="🌳 Da esplorare"/>
        <Row l="🌳 Parco S'Ollivariu" v="~5 min ›" link="https://maps.google.com/?q=Parco+S+Ollivariu+Uta"/>
        <Row l="⛪ Chiesa Santa Maria (romanica)" v="~5 min ›" link="https://maps.google.com/?q=Chiesa+Santa+Maria+Uta"/>
        <Row l="🦜 Oasi WWF Santa Gilla" v="~10 min ›" link="https://maps.google.com/?q=Oasi+WWF+Santa+Gilla"/>
        <Row l="🏖️ Poetto — Cagliari" v="~15 min ›" link="https://maps.google.com/?q=Spiaggia+Poetto+Cagliari"/>
        <Row l="🏙️ Cagliari centro storico" v="~20 min ›" link="https://maps.google.com/?q=Cagliari+centro+storico"/>
